...
// Shared PDF generator — renders an HTML document string into a hidden
// off-screen iframe (so external images actually load and the page's own
// styles don't interfere), then captures it to a real downloadable PDF
...
function buildOutstandingCandidateHtml(task, coach) {
  ...
  return `<html><head><title>${esc(task.coachName)} - Outstanding Items</title></head><body style="margin:0;font-family:-apple-system,Helvetica,Arial,sans-serif;color:#1e293b;padding:28px;">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;border-bottom:2px solid #0f172a;padding-bottom:16px;margin-bottom:20px;">
      <div style="width:44px;height:44px;border-radius:9px;background:#fff;border:1px solid #e2e8f0;display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden;"><img src="/api/logo-proxy?url=${encodeURIComponent(memberFederationLogo(task.memberFederation))}" alt="${esc((MEMBER_FEDERATIONS.find(m => m.key === task.memberFederation) || {}).label || "Football Victoria")} logo" style="max-width:100%;max-height:100%;object-fit:contain;" /></div>
      <div style="flex:1;">
      ...
      ${task.memberFederation !== "FA" ? `<div style="width:44px;height:44px;border-radius:9px;background:#fff;border:1px solid #e2e8f0;display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden;"><img src="/api/logo-proxy?url=${encodeURIComponent((MEMBER_FEDERATIONS.find(m => m.key === "FA") || {}).logoUrl || "")}" alt="Football Australia logo" style="max-width:100%;max-height:100%;object-fit:contain;" /></div>` : ""}
    </div>
...
function CompletedTasksTab({ coaches, courses, saveCoaches, completedTasks, saveCompletedTasks, onBulkDelete, observations, onViewReport, closedCourseNumbers, saveClosedCourseNumbers, closedCourseBlocks, saveClosedCourseBlocks, adminSettings, adminLockouts, recordAdminAttempt, session }) {
  ...
  async function handleSaveCandidatePdf(t, cardCoach) {
    ...
    try {
      const filename = `${(t.coachName || "candidate").replace(/[^a-z0-9]+/gi, "-")}-report.pdf`;
      await saveHtmlAsPdf(buildCandidateHtml(t, cardCoach, observations), filename);
    } catch (e) {
                                      ...
                                      <div className="mt-3 pt-3 border-t border-slate-100 space-y-3">
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                          <button onClick={() => handleSaveCandidatePdf(t, cardCoach)} disabled={savingCandidatePdfId === t.id}
                                            className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 border border-emerald-300 px-2.5 py-1.5 rounded-lg hover:bg-emerald-50 disabled:opacity-50">
                                            <FileText className="w-3.5 h-3.5" /> {savingCandidatePdfId === t.id ? "Saving..." : `Save PDF for ${t.coachName}`}
                                          </button>
...
function ReportView({ observations, reportId, coaches, onBack, onEditDraft, onSubmitDraft }) {
  ...
  async function handleSavePdf() {
    ...
    try {
      ...
      await saveHtmlAsPdf(htmlContent, filename);
    } catch (e) {
